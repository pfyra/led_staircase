...
void NewRemoteTransmitter::sendGroup(unsigned long address, boolean switchOn) {
  _address = address;
...
void NewRemoteTransmitter::sendUnit(unsigned long address, byte unit, boolean switchOn) {
	_address = address;
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::sendDim(unsigned long address, byte unit, byte dimLevel) {
  _address = address;
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::sendGroupDim(unsigned long address, byte dimLevel) {
  _address = address;
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
