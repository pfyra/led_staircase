...
class NewRemoteTransmitter {
	public:
		...
		*/
		 ...
		 */
		void sendGroup(unsigned long address, boolean switchOn);
		 ...
		 * @param switchOn  True to send "on" signal, false to send "off" signal.
		 */
		void sendUnit(unsigned long address, byte unit, boolean switchOn);
		 ...
		 */
		void sendDim(unsigned long address, byte unit, byte dimLevel);
		 ...
		 * @param dimLevel  [0..15] Dim level. 0 for off, 15 for brightest level.
		 */
		void sendGroupDim(unsigned long address, byte dimLevel);
